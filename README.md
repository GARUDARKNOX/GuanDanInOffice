...
*   **智能 AI 托管 (Bot AI v5)**：人数不足时，智能 Bot 自动补位；采用多层级决策引擎，含阶段感知、炸弹矩阵、终局保护、联盟配合等策略
*   **完整对局系统 (Match System)**：
...
Bot AI 采用**多层级策略系统**，模拟人类掼蛋高手的决策方式：

### 阶段感知 (Phase Awareness)
| 阶段 | 策略 |
|------|------|
| **开局** (>20张) | 情况不明对子先行——出倒数第二大对子探路 |
| **中期** (10-20张) | 逐轮出最弱组合，保留强牌控盘 |
| **残局** (≤10张) | 计算对手剩余牌数，实施终局保护 |

### 终局保护
- **下家敌人剩1张** → 绝对不出单张；被迫出则出最大单
- **下家敌人剩≤3张** → 优先出非单张组合
...
## 📝 更新日志
*   **v5.0** (2026-06):
    *   🤖 **Bot AI v5 全面升级**：多层级决策引擎，含阶段感知、终局保护、炸弹矩阵、角色定位、联盟配合
    *   🧠 开局策略：倒数第二大对子先行，不暴露最弱牌
    *   💣 炸弹决策矩阵：5/7张必炸、4/8张不炸、枪不打四
    *   🛡️ 终局保护：下家敌人剩1张不出单张
    *   🤝 联盟配合：识别头游走牌自动过牌不压
    *   ✅ 出牌前验证：getHandType()校验合法性，杜绝非法牌型
*   **v2.0** (2026-01):
    *   ✨ 新增**技能掼蛋模式**，包含5种技能卡。
