...
export function useGame() {
  ...
  // Track gameOver timeout so we can cancel it when new gameState arrives
  ...
  useEffect(() => {
    ...
    socket.on('gameState', (state: GameState) => {
      ...
      // Cancel any pending gameOver clear timer — new game has started
    ...
    socket.on('gameOver', (data: { winners: number[] }) => {
      ...
      // Clear gameState after 2.5s to remove Score overlay.
      // Server starts next game after 3s, so new gameState arrives ~0.5s later.
      // If new gameState arrives before timer fires, it will be cancelled.
    ...
    return () => {
      // Clear any pending gameOver timer
      if (gameOverTimerRef.current) {
        clearTimeout(gameOverTimerRef.current);
      }
