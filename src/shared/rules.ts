...
/** Build rank → count map from non-wild cards */
function buildRankCounts(nonWilds: Card[]): Map<number, number> {
...
/** Try to match cards as Tube (钢板): 3 consecutive ranks × 2 cards each */
export function tryMatchTube(cards: Card[], nonWilds: Card[], wildCount: number): Hand | null {
  const rankCounts = buildRankCounts(nonWilds);
  for (const ranks of TUBE_STARTS) {
    const rankSet = new Set(ranks);
    let wildsNeeded = 0;
    let valid = true;
    for (const [r, count] of rankCounts) {
      if (!rankSet.has(r)) { valid = false; break; }
      if (count > 2) { valid = false; break; }
    }
    if (!valid) continue;
    for (const r of ranks) {
      const count = rankCounts.get(r) || 0;
      if (count < 2) wildsNeeded += (2 - count);
    }
    if (wildsNeeded <= wildCount) {
      const value = ranks[0] === 14 ? 3 : ranks[2];
      return { type: HandType.Tube, cards, value };
    }
  }
  return null;
}

/** Try to match cards as Plate (木板): 2 consecutive ranks × 3 cards each */
export function tryMatchPlate(cards: Card[], nonWilds: Card[], wildCount: number): Hand | null {
  const rankCounts = buildRankCounts(nonWilds);
  for (const ranks of PLATE_STARTS) {
    const rankSet = new Set(ranks);
    let wildsNeeded = 0;
    let valid = true;
    for (const [r, count] of rankCounts) {
    ...
    for (const r of ranks) {
      const count = rankCounts.get(r) || 0;
      if (count < 3) wildsNeeded += (3 - count);
    }
    if (wildsNeeded <= wildCount) {
      return { type: HandType.Plate, cards, value: ranks[1] };
    }
  }
  return null;
}
...
export function getHandType(cards: Card[], level: number): Hand | null {
  ...
  // 8. Tube (6 cards) - 3 consecutive pairs (钢板)
  // 9. Plate (6 cards) - 2 consecutive trips (木板)
  if (len === 6) {
      const tubeResult = tryMatchTube(cards, nonWilds, wildCount);
      if (tubeResult) return tubeResult;
      const plateResult = tryMatchPlate(cards, nonWilds, wildCount);
...
export function getAllPossibleHandTypes(cards: Card[], level: number): Hand[] {
    ...
    // Use getHandType as starting point (now supports Tube/Plate with wilds)
    const defaultHand = getHandType(cards, level);
    if (defaultHand) {
        results.push(defaultHand);
    }
    
    // For 6-card hands with wilds, also try Tube and Plate interpretations
    // that getHandType might have missed (e.g., Bomb takes priority over Tube)
    if (len === 6 && wilds.length > 0) {
        // Use shared detection functions
        const tubeResult = tryMatchTube(cards, nonWilds, wilds.length);
        if (tubeResult) {
          const existing = results.find(r => r.type === HandType.Tube && r.value === tubeResult.value);
          if (!existing) results.push(tubeResult);
        }
        const plateResult = tryMatchPlate(cards, nonWilds, wilds.length);
        if (plateResult) {
          const existing = results.find(r => r.type === HandType.Plate && r.value === plateResult.value);
          if (!existing) results.push(plateResult);
        }
    }
